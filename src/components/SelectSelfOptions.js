...
export default function SelectSelfOptions(props) {
  ...
  const handleNextButtonClick = () => {
    props.onClick('ResultList', optionsSelected);
  }

  return (
    <div>
      I need at least <Input placeholder='0' onChange={(event) => optionsSelected.minData = event.target.value }/> GB of data, <br/>
      at least <Input placeholder='0' onChange={(event) => optionsSelected.minTalktime = event.target.value }/> min of talktime, <br/>
      and at least <Input placeholder='0' onChange={(event) => optionsSelected.minSMS = event.target.value }/> SMS. <br/>
      My monthly budget for this will be $<Input placeholder='0' onChange={(event) => optionsSelected.price = event.target.value }/><br/>
      <Button onClick={() => handleNextButtonClick()} variant='outlined' size='large' color='primary'>Next</Button>
    </div>
  );
}
