...
const ResultsList = props => {
  ...
  const filteredMobilePlans = mobilePlanData.filter(
    mobilePlan =>
  ...
  const unfilteredMobilePlans = mobilePlanData.filter(
    mobilePlan =>
      (mobilePlan.data < options.minData ||
        mobilePlan.talktime < options.minTalktime ||
        mobilePlan.sms < options.minSMS ||
  ...
  for (const mobilePlan of unfilteredMobilePlans) {
    ...
    const addonsForPlan = addonsData.filter(
      addon =>
        addon.appliesToTelco === mobilePlan.telco &&
        (addon.appliesToPlans === 'All' ||
          addon.appliesToPlans.includes(mobilePlan.planName))
    ); //find suitable addons, i.e. those that apply to entire telco or those that only apply to the mobile plan
    for (let x = 0; x < addonsForPlan.length; x++) {
      ...
      for (let y = x; y < addonsForPlan.length; y++) {
        ...
        if (addon.multiplier === true) {
          ...
          const addonMultipleForData = Math.ceil(
            (options.minData - newPlan.data) / addon.data
          ); //check how many data addons are required
          const addonMultipleForTalktime = Math.ceil(
            (options.minTalktime - newPlan.talktime) / addon.talktime
          ); //check how many talktime addons are required
          const addonMultipleForSMS = Math.ceil(
            (options.minSMS - newPlan.sms) / addon.sms
          ); //check how many sms addons are required
          const addonMultiple = Math.max(
            addonMultipleForData,
            addonMultipleForTalktime,
            addonMultipleForSMS
          );
        ...
        if (
          ...
          const addonPros =
            newPlan.addons.pros !== undefined
              ? newPlan.addons.reduce(
                  (allAddonPros, addonPros) => allAddonPros.concat([addonPros]),
                  []
                ).pros
              : null;
          const addonCons =
            newPlan.addons.cons !== undefined
              ? newPlan.addons.reduce(
                  (allAddonCons, addonCons) => allAddonCons.concat([addonCons]),
                  []
                ).cons
              : null;
          const addonNotes =
            newPlan.addons.notes !== undefined
              ? newPlan.addons.reduce(
                  (allAddonNotes, addonNotes) =>
                    allAddonNotes.concat([addonNotes]),
                  []
                ).notes
              : null;
  ...
  const sortedMobilePlans = filteredMobilePlans.sort((a, b) =>
    a.price < b.price ? -1 : 1
  ); //sort by cheap to expensive
